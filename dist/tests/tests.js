...
module.exports = function (test, Where) {
	test('= : equals', function (t) {
		...
		t.ok(!evaluate(record, 'where a = c'), "equals evaluation on unequal fields");

		t.ok(evaluate(record, 'where a = 1'), "equals evaluation on field and constant");
		t.ok(!evaluate(record, 'where a = -2'), "equals evaluation on unequal field and constant");

		t.ok(evaluate(record, 'where -1 = c'), "equals evaluation on constant and field");
		t.ok(!evaluate(record, 'where 2 = c'), "equals evaluation on unequal constant and field");

		t.ok(evaluate(record, 'where 0.1 = 0.1'), "equals evaluation on constants");
		t.ok(!evaluate(record, 'where 0.1 = -0.2'), "equals evaluation on unequal constants");

		t.ok(evaluate(record, 'where d = d'), "equals evaluation on null field with itself");
		t.ok(!evaluate(record, 'where d = c'), "equals evaluation on null and non-null fields");

		t.ok(evaluate(record, 'where null = null'), "equals evaluation on null constants");
		t.ok(!evaluate(record, 'where null = 1'), "equals evaluation on null and non-null constants");
		...
		t.ok(!evaluate(record, 'where c = null'), "equals evaluation on non-null field and null constant");
		t.ok(!evaluate(record, 'where null = c'), "equals inverse evaluation on non-null field and null constant");
	...
	test('!= : not equals', function (t) {
		...
		t.ok(!evaluate(record, 'where a != c'), "not equals evaluation on equal fields");

		t.ok(evaluate(record, 'where a != 1'), "not equals evaluation on field and constant");
		t.ok(!evaluate(record, 'where a != -1'), "not equals evaluation on equal field and constant");

		t.ok(evaluate(record, 'where 1 != c'), "not equals evaluation on constant and field");
		t.ok(!evaluate(record, 'where -1 != c'), "not equals evaluation on equal constant and field");

		t.ok(evaluate(record, 'where 0.1 != -0.2'), "not equals evaluation on constants");
		t.ok(!evaluate(record, 'where 0.1 != 0.1'), "not equals evaluation on equal constants");

		t.ok(!evaluate(record, 'where d != d'), "not equals evaluation on null field with itself");
		t.ok(evaluate(record, 'where d != c'), "not equals evaluation on null and non-null fields");

		t.ok(!evaluate(record, 'where null != null'), "not equals evaluation on null constants");
		t.ok(evaluate(record, 'where null != 1'), "not equals evaluation on null and non-null constants");

		t.ok(!evaluate(record, 'where d != null'), "not equals evaluation on null field and constant");
		t.ok(!evaluate(record, 'where null != d'), "not equals inverse evaluation on null field and constant");
	...
	test('< : less than', function (t) {
		...
		t.ok(!evaluate(record, 'where b < a'), "less than inverse evaluation on fields");

		t.ok(evaluate(record, 'where a < 0'), "less than evaluation on field and constant");
		t.ok(!evaluate(record, 'where 0 < a'), "less than inverse evaluation on field and constant");

		t.ok(evaluate(record, 'where 0 < b'), "less than evaluation on constant and field");
		t.ok(!evaluate(record, 'where b < 0'), "less than inverse evaluation on constant and field");

		t.ok(evaluate(record, 'where -0.2 < 0.1'), "less than evaluation on constants");
		t.ok(!evaluate(record, 'where 0.1 < -0.2'), "less than inverse evaluation on constants");
		t.ok(!evaluate(record, 'where 0 < 0'), "less than evaluation on equal constants");
		if (typeof t.end === 'function') t.end();
	});
	...
	test('<= : less than or equal', function (t) {
		...
		t.ok(!evaluate(record, 'where b <= a'), "less than or equal inverse evaluation on fields");

		t.ok(evaluate(record, 'where a <= 0'), "less than or equal evaluation on field and constant");
		t.ok(!evaluate(record, 'where 0 <= a'), "less than or equal inverse evaluation on field and constant");
		...
		t.ok(!evaluate(record, 'where b <= 0'), "less than or equal inverse evaluation on constant and field");
		...
		t.ok(!evaluate(record, 'where 0.1 <= -0.2'), "less than or equal inverse evaluation on constants");
	...
	test('> : greater than', function (t) {
		...
		t.ok(!evaluate(record, 'where b > a'), "greater than inverse evaluation on fields");

		t.ok(evaluate(record, 'where a > 0'), "greater than evaluation on field and constant");
		t.ok(!evaluate(record, 'where 0 > a'), "greater than inverse evaluation on field and constant");

		t.ok(evaluate(record, 'where 0 > b'), "greater than evaluation on constant and field");
		t.ok(!evaluate(record, 'where b > 0'), "greater than inverse evaluation on constant and field");

		t.ok(evaluate(record, 'where 0.1 > -0.2'), "greater than evaluation on constants");
		t.ok(!evaluate(record, 'where -0.2 > 0.1'), "greater than inverse evaluation on constants");
		t.ok(!evaluate(record, 'where 0 > 0'), "greater than evaluation on equal constants");
		if (typeof t.end === 'function') t.end();
	});
	...
	test('>= : greater than or equal', function (t) {
		...
		t.ok(!evaluate(record, 'where b >= a'), "greater than or equal inverse evaluation on fields");

		t.ok(evaluate(record, 'where a >= 0'), "greater than or equal evaluation on field and constant");
		t.ok(!evaluate(record, 'where 0 >= a'), "greater than or equal inverse evaluation on field and constant");
		...
		t.ok(!evaluate(record, 'where b >= 0'), "greater than or equal inverse evaluation on constant and field");
		...
		t.ok(!evaluate(record, 'where -0.2 >= 0.1'), "greater than or equal inverse evaluation on constants");
	...
	test('in : in array', function (t) {
		...
		t.ok(!evaluate(record, 'where d in [-1, 0, 1]'), "in array evaluation on field to non-existent element in array");
		t.ok(!evaluate(record, 'where "a string" in [-1, 0, 1]'), "in array evaluation on constant to non-existent element in array");
